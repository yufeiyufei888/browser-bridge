...
name: browser-bridge
description:
  用真实浏览器（Chrome / Edge / Chromium，自带登录态）完成联网与网页操作。触发场景：需要登录态或动态渲染的页面、反爬站点、页面内点击/填表/上传/翻页等交互、
  网页截图与视频截帧、读取本地浏览器书签/历史，以及 Agent 自带 WebSearch/WebFetch 拿不到内容时的兜底。静态页面优先用 WebSearch/WebFetch（更省 token），本技能只负责需要真实浏览器的那部分。
license: MIT
metadata:
  version: "1.0.0"
  upstream: https://github.com/eze-is/web-access
  upstream_commit: 33eef84a55b1919396a80e7a55650a07bb83f590
  packaging: browser-bridge (cross-platform / cross-agent distribution)
---

> ## 🔌 适配与使用前提（跨平台 / 跨 Agent）
>
> **1. 解析技能根目录** —— 下文一律记作 `<SKILL_DIR>`，执行命令前请替换成真实绝对路径：
> - Claude Code / Codex：`$CLAUDE_SKILL_DIR`（POSIX）/ `%CLAUDE_SKILL_DIR%`（Windows）
> - DSH：加载时注入的 `Base directory for this skill: ...`
> - 其他 Agent：你读取本 `SKILL.md` 的目录
>
> **2. 两种 shell 方言 —— 先确认自己在哪一边，再抄对应命令**
>
> | 场景 | POSIX（bash/zsh） | Windows PowerShell 5.1 |
> |---|---|---|
> | HTTP 调用 | `curl` | **必须写 `curl.exe`**（`curl` 是 `Invoke-WebRequest` 别名，直接抄会报参数错） |
> | 结束 proxy | `pkill -f cdp-proxy.mjs` | 用 `Get-CimInstance Win32_Process` 匹配 `cdp-proxy` 后 `Stop-Process`（见「前置检查」） |
> | 传 JS（含引号/中文/多行） | `--data-raw '…'` | 写入 **UTF-8 无 BOM** 文件后 `--data-binary "@文件路径"` |
>
> **3. 环境要求**：Node.js **22+**（用原生 WebSocket，零 npm 依赖）；Chrome / Edge / Chromium 任一，需开启远程调试（见「前置检查」）。
>
> **4. 不要接管全部联网**：Agent 自带的 WebSearch / WebFetch 处理静态页更省 token。只有需要登录态、动态渲染、反爬、页面交互、截图/视频帧，或前两者拿不到内容时，才走本技能的 CDP。
>
> **5. 已知限制与排错**：见文末同名章节（截图与窗口最小化、`find-url` 的 sqlite3 依赖等）。
>
> 本技能改编自 [web-access](https://github.com/eze-is/web-access)（MIT · 一泽Eze），脚本层逐字节沿用上游，详见 `NOTICE.md`。
...
切换浏览器时，proxy 是长驻进程，需先结束它再重跑 check-deps（Windows 没有 `pkill`）：

```powershell
Get-CimInstance Win32_Process -Filter "Name='node.exe'" | Where-Object { $_.CommandLine -match 'cdp-proxy' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }
```
...
所有操作通过 HTTP 调用 proxy；下面按 **Windows PowerShell** 写（`curl.exe`），POSIX 用户把 `curl.exe` 换成 `curl`、把 `--data-binary "@文件"` 换成 `--data-raw '表达式'` 即可：

```powershell
$P = "http://localhost:3456"

# 列出用户已打开的 tab
curl.exe -s "$P/targets"

# 创建新后台 tab（URL 走 POST body，避免目标 URL 含 query 时被切分）
curl.exe -s -X POST --data-raw "https://example.com" "$P/new"

# 页面信息
curl.exe -s "$P/info?target=ID"

# 执行任意 JS：可读写 DOM、提取数据、操控元素、触发状态变更、提交表单、调用内部方法
curl.exe -s -X POST --data-raw "document.title" "$P/eval?target=ID"

# JS 含引号/空格/中文或多行时 —— 写成 UTF-8 无 BOM 文件再 --data-binary "@文件"（最稳，推荐默认这么做）
...
curl.exe -s -X POST --data-raw "https://example.com" "$P/navigate?target=ID"
curl.exe -s "$P/back?target=ID"

# 点击（POST body 为 CSS 选择器）— JS el.click()，简单快速，覆盖大多数场景
curl.exe -s -X POST --data-raw "button.submit" "$P/click?target=ID"

# 真实鼠标点击 — CDP Input.dispatchMouseEvent，算用户手势，能触发文件对话框
curl.exe -s -X POST --data-raw "button.upload" "$P/clickAt?target=ID"

# 文件上传 — 直接设置 file input 的本地文件路径，绕过文件对话框（JSON 里有双引号，同样建议走文件）
[IO.File]::WriteAllText("$env:TEMP\wa-files.json", '{"selector":"input[type=file]","files":["C:\\Users\\yufei\\Desktop\\a.png"]}', (New-Object Text.UTF8Encoding($false)))
curl.exe -s -X POST --data-binary "@$env:TEMP\wa-files.json" "$P/setFiles?target=ID"

# 滚动（触发懒加载）
curl.exe -s "$P/scroll?target=ID&y=3000"
curl.exe -s "$P/scroll?target=ID&direction=bottom"

# 关闭 tab
curl.exe -s "$P/close?target=ID"
```
...
- 必须在子 Agent prompt 中写 `必须加载 browser-bridge skill 并遵循指引` ，子 Agent 会自动加载 skill，无需在 prompt 中复制 skill 内容或指定路径。
...
| 现象 | 原因 / 处理 |
|---|---|
| `/screenshot` 长时间不返回（CDP 命令超时 30s） | 目标页所在浏览器窗口被**最小化/遮挡**时，Chrome/Edge 不再产生合成帧。先把浏览器窗口还原再截图；固定解法见下 |
| 长期需要后台截图 | 给浏览器快捷方式加 `--disable-backgrounding-occluded-windows --disable-features=CalculateNativeWinOcclusion` 后重启（保留 inspect toggle 与登录态） |
| `find-url.mjs` 报找不到 `sqlite3` | 该脚本依赖外部 `sqlite3` CLI 读取书签/历史。Windows：`winget install sqlite.sqlite`；macOS：`brew install sqlite` |
| Windows 上 `curl` 报参数错误 | 你用的是 `Invoke-WebRequest` 别名，改 `curl.exe` |
| PowerShell 里 `--data-raw` 的引号被吃掉 | 改走 UTF-8 无 BOM 文件 + `--data-binary "@file"` |
| `check-deps` 报「浏览器不一致」并硬错 | 3456 端口已被**另一个浏览器**的 proxy 占用。先结束旧 proxy（`pkill` / `Stop-Process`）再重跑；或用 `config.env` 固定浏览器 |
| 每次新连接都弹「是否允许远程调试」 | Chrome/Edge 对**每个新的调试客户端**都要授权。用本技能常驻的 proxy 复用连接，不要自己另开 WebSocket 直连调试端口 |

## 归属与许可

- 上游：[web-access](https://github.com/eze-is/web-access) · MIT · 作者 **一泽Eze**（commit `33eef84`）
- 本发行版：**browser-bridge** —— 跨平台 / 跨 Agent 打包与文档适配；`scripts/`、`references/`、`templates/` 与上游逐字节一致，未改动逻辑
- 许可：MIT，详见 `LICENSE` 与 `NOTICE.md`
## References 索引
